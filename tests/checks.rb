...
def good_ticker(ticker)
  good_params(ticker,
              %w[low high volume volume_quote timestamp])
end

# goodTicker check the precence of every field in the ticker dict
def good_price(price)
  good_params(price,
              %w[currency price timestamp])
end
...
def good_history_point(point)
  good_params(point,
              %w[open close min max timestamp])
end
...
def good_candle(candle)
  good_params(candle,
              %w[timestamp open close min max volume volume_quote])
end

def good_balance(balance)
  good_params(balance,
              %w[currency available reserved])
end
...
def good_trade(trade)
  good_params(trade,
              %w[id orderId clientOrderId symbol side quantity
                 price fee timestamp])
end

def good_native_transaction(native_transaction)
  good_params(native_transaction,
              %w[tx_id index currency amount])
end
